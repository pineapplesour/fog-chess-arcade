...
| 가운데 버튼 드래그 | 시야를 돌린다 |
| 휠 | 카메라 거리 |
| F → 칸 클릭 | 조명탄. 원하는 5×5 구역을 한 턴 밝힌다 |
| N | 새 게임 |
| 제한시계 | 시간이 다하면 손이 미끄러져 임의의 수가 나간다 |

화면에는 별도 UI가 없다. 점수·상대·조명탄·제한시계는 전부 카메라 화면 안 OSD로 표시된다.
...
6. **자기 킹 안전** — Stockfish에는 "킹이 잡힌다"는 개념이 없다(표준 체스에선 불가능한 수라서).
   그래서 후보 수를 샘플 보드에 실제로 둬 보고, 상대가 다음 수에 내 킹을 잡을 수 있으면
   그 세계 점수를 −9000으로 덮는다. 한 세계에서라도 킹이 날아가는 수는 추가로 크게 깎는다.
7. **정찰 가치** — 값이 비슷하면 시야가 더 넓어지는 수를 고른다. 안개 체스에서 시야는 자원이다.

8. **신념 갱신** — 못 본 기물에는 나이를 매기고, 샘플마다 한두 개만 한 수 분량으로 흘려보낸다.
   이걸 안 하면 상대 군대가 초기 배치에 얼어붙은 유령이 되어 평가가 통째로 어긋난다.
   반대로 전부 흘려보내면 군대가 흩어져 노이즈가 된다(실측으로 확인).

### 계산량과 강도 (상대: 신념 보드 3수 탐색 봇, 시계 없음)

| 샘플 × 탐색시간 | 전적 | 한 수 대기 |
|---|---|---|
| 14 × 70ms | 6승 3패 | 3.5초 |
| **18 × 140ms (현재 기본값)** | **4승 0패** | 5.9초 |
| 24 × 200ms | 6승 0패 | 7.6초 |

계산량에 비례해 강해진다. 논문의 초인적 성능은 이보다 수백 배 많은 탐색을 쓴다.
